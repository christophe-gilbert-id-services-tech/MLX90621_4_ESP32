...
void MLX90621::initialise(int refrate) {
	refreshRate = refrate;
	Wire.begin(); // CGI / I2C_MASTER, 0, I2C_PINS_18_19, I2C_PULLUP_INT, I2C_RATE_100
	delay(5);
	readEEPROM();
	writeTrimmingValue();
	setConfiguration();
...
void MLX90621::readEEPROM() { // Read in blocks of 32 bytes to accomodate Wire library
  for(int j=0;j<256;j+=32) {
    ...
    Wire.write(j);
    byte rc = Wire.endTransmission(false); // CGI I2C_NOSTOP
    if(rc) {
...
void MLX90621::readIR() {
  for(int j=0;j<64;j+=16) { // Read in blocks of 32 bytes to overcome Wire buffer limit   
    ...
    Wire.write(0x01);
    Wire.write(0x20);
    Wire.endTransmission(false); // CGI I2C_NOSTOP
...
void MLX90621::readPTAT() {
	...
	Wire.write(0x00);
	Wire.write(0x01);
	Wire.endTransmission(false); // I2C_NOSTOP
